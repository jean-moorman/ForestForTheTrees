...
class TestPhaseOneFireIntegration:
    ...
    def phase_one_workflow(self, mock_agents, mock_resources):
        ...
        return PhaseOneWorkflow(
            garden_planner_agent=mock_agents["garden_planner"],
            earth_agent=mock_agents["earth_agent"],
            environmental_analysis_agent=mock_agents["environmental_analysis"],
            root_system_architect_agent=mock_agents["root_system_architect"],
            tree_placement_planner_agent=mock_agents["tree_placement_planner"],
...
class TestPhaseThreeFireAirIntegration:
    ...
    def natural_selection_agent(self, mock_natural_selection_resources):
        ...
        with patch('phase_three.evolution.natural_selection.FeatureRequirementsAnalysisAgent'), \
             patch('phase_three.evolution.natural_selection.FeatureImplementationAnalysisAgent'), \
             patch('phase_three.evolution.natural_selection.FeatureEvolutionAgent'):
            
            agent = NaturalSelectionAgent(
                event_queue=mock_natural_selection_resources["event_queue"],
                state_manager=mock_natural_selection_resources["state_manager"],
                context_manager=mock_natural_selection_resources["context_manager"],
                cache_manager=mock_natural_selection_resources["cache_manager"],
                metrics_manager=mock_natural_selection_resources["metrics_manager"],
                error_handler=mock_natural_selection_resources["error_handler"]
            )
            
            # Mock the phase zero agents
            agent._requirements_analysis_agent.process_with_validation = AsyncMock(return_value={"analysis": "requirements"})
            agent._implementation_analysis_agent.process_with_validation = AsyncMock(return_value={"analysis": "implementation"})
            agent._evolution_agent.process_with_validation = AsyncMock(return_value={"analysis": "evolution"})
            
            # Mock the main agent methods
            agent.set_agent_state = AsyncMock()
            agent.process_with_validation = AsyncMock(return_value={
                "feature_rankings": [
                    {"feature_id": "complex_feature_1", "rank": 2, "overall_score": 0.65},
                    {"feature_id": "simple_feature_2", "rank": 1, "overall_score": 0.85},
                    {"feature_id": "medium_feature_3", "rank": 3, "overall_score": 0.72}
                ],
                "optimization_decisions": [
                    {"feature_id": "complex_feature_1", "decision": "improve", "rationale": "High complexity but good potential"},
                    {"feature_id": "simple_feature_2", "decision": "keep", "rationale": "Excellent performance"},
                    {"feature_id": "medium_feature_3", "decision": "keep", "rationale": "Good performance"}
                ],
                "evolution_strategy": {
                    "reuse_opportunities": [],
