...
class TestEventQueueThreadSafety:
    ...
    async def test_concurrent_emission(self, event_queue):
        ...
        async def event_handler(event_type, data):
            ...
            with event_lock:
                ...
                if len(received_events) >= 100:
                    event_received.set()
                    print(f"All 100 events received")
        ...
        def emit_events(thread_id):
            ...
            # Emit 20 events with small delay between them
            for i in range(20):
                try:
                    loop.run_until_complete(
                        event_queue.emit("test_event", {
                            "thread_id": thread_id,
                            "sequence": i
                        })
                    )
        ...
        assert event_count >= 20, f"Expected at least 20 events, got {event_count}\nReceived events: {received_events[:5]}..."
        
        # Verify events from multiple threads were received (at least 3 out of 5)
        thread_ids = set()
        for _, data in received_events:
            thread_ids.add(data.get('thread_id'))
        
        assert len(thread_ids) >= 3, f"Expected events from at least 3 threads, got events from {len(thread_ids)} threads: {thread_ids}"
        ...
        assert len(thread_ids) == 5, f"Events not received from all threads: {thread_ids}"
