...
class TestEventQueue:
    ...
    async def test_queue_property_lazy_initialization(self):
        """Test that the queue property initializes lazily."""
        queue = EventQueue()
        # Queue should be None initially
        assert queue._queue is None
        
        # Accessing the queue property should initialize it
        actual_queue = queue.queue
        assert actual_queue is not None
        assert queue._creation_loop_id is not None
        assert queue._loop_thread_id is not None
        ...
        def run_in_thread():
            async def async_task():
                ...
                try:
                    ...
                    # Return results
                    return {
                        "queue_created": thread_queue._queue is not None,
