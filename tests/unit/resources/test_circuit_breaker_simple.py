...
async def test_concurrent_circuit_operations(circuit_registry):
    ...
    async def execute_and_catch(index):
        try:
            await circuit_registry.circuit_execute(
                "shared_circuit", 
                lambda: failure_operation(index)
            )
        except Exception as e:
            results.append(f"caught-{index}")
    
    failure_tasks = []
    for i in range(5):
        task = asyncio.create_task(execute_and_catch(i))
