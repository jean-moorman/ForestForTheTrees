...
class TestMemoryMonitor:
    ...
    async def test_track_resource(self, memory_monitor, event_queue):
        ...
        # Register a component with lower thresholds for easier testing
        custom_thresholds = MemoryThresholds(
            warning_percent=0.5,
            critical_percent=0.8,
            per_resource_max_mb=50.0  # Lower threshold for testing
        )
        memory_monitor.register_component("test_component", custom_thresholds)
        ...
        # Track a resource above threshold to trigger alert (use the component's threshold)
        resource_size = custom_thresholds.per_resource_max_mb + 10
        logger.info(f"Tracking resource with size {resource_size}MB > threshold {custom_thresholds.per_resource_max_mb}MB")
